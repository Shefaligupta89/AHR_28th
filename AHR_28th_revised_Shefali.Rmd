...
# Load the "Scores & Rankings" sheet from the Excel workbook
scores_rankings <- read_excel("C:/Shefali/Reason Foundation/Highway report_28th/ahr_data_28th.xlsx", sheet = "Scores & Rankings")

# Define the function to generate colored bars proportional to the score
create_bar <- function(value, max_value, color) {
  # Proportional width of the bar
  width <- paste0((value / max_value) * 100, "%")
  
  # Ensuring consistent height, flexbox alignment, and no text wrapping
  div(style = "display: flex; align-items: center; white-space: nowrap;",  # No text wrapping, and keeping alignment
      div(style = paste("width:", width, "; background-color:", color, "; height: 15px; margin-right: 5px;", sep = "")),  # Bar size control
      div(sprintf("%.2f", value))  # Display the value next to the bar
  )
}
...
reactable(
  ...
  columns = list(
    ...
    overall_score = colDef(
      ...
      minWidth = 150  # Adjust width of column to avoid text wrapping
    ),
    ...
    state_avg_congestion_hours_score = colDef(
      name = "Congestion Hours Score",
      cell = function(value) {
        create_bar(value, max_congestion_hours, "#FFCC33")  # Yellow for congestion hours score
      },
      minWidth = 150  # Adjust width of column to avoid text wrapping
    ),
...
# Create a simple reactable table to show rank and state only
reactable(
  data = score_table,
  filterable = FALSE, # No need for filtering
  columns = list(
  ...
  defaultPageSize = 10,  # Adjust as needed
  striped = TRUE,  # Striped rows for better readability
  highlight = TRUE,  # Highlight selected row
  bordered = TRUE,  # Add border for better visuals
  paginationType = "simple",  # Simple pagination style
  defaultSorted = list(overall_score_rank = "asc")  # Sort by rank in ascending order
)
...
# Load necessary libraries
library(dplyr)
library(reactable)  # For creating interactive tables
library(htmltools)  # For creating div elements for bar charts

# Prepare the data table using only the overall rank and state, and sort alphabetically by state
score_table_alphabetical <- scores_rankings %>%
  select(state, overall_score_rank) %>%
  arrange(state)  # Sort alphabetically by state

# Get the max value for overall score rank to scale the bar chart
max_score_rank <- max(score_table_alphabetical$overall_score_rank, na.rm = TRUE)

# Function to create a bar chart in the cell
create_bar_chart <- function(value, max_value, color = "#FFCC33") {
  ...
  htmltools::div(style = bar_style, format(value, big.mark = ",", scientific = FALSE))  # Display the value inside the bar
}

# Create a reactable table with the bar chart for the overall score rank
reactable(
...
library(usmap)  # For simplified U.S. state maps
...
# Prepare the data: select state and overall rank, convert state names to lowercase to match map data
score_table <- scores_rankings %>%
  select(state, overall_score_rank) %>%
  mutate(state = tolower(state))  # Convert state names to lowercase to match with map data

# Get US state map data from the 'maps' package
states_map <- map_data("state")

# Merge the map data with your rankings data based on state names
map_data <- left_join(states_map, score_table, by = c("region" = "state"))

# Calculate the centroids manually by averaging lat and long for each state
...
ggplot() +
  ...
  theme_minimal() +
  # Add state abbreviations and ranks at centroids
...
library(dplyr)
library(reactable)  # For creating interactive tables
library(htmltools)  # For creating div elements for bar charts and adding title

# Prepare the data: select state and total lane miles, filter out "United States", then rank them
...
# Calculate total lane miles for the U.S. and add it as a new row
us_total <- data.frame(
...
# Append the "U.S. Total" row at the end by manually binding it
lane_miles_table <- bind_rows(lane_miles_table, us_total)

# Max value for lane miles to use for bar chart scaling
max_lane_miles <- max(lane_miles_table$state_tot_lane_miles, na.rm = TRUE)

# Function to create a bar chart in the cell with centrally aligned numbers and bars
create_bar_chart <- function(value, max_value, color = "#66bb6a") {
  if (is.na(value)) return("")  # Skip the "U.S. Total" row
  width_percent <- 100 * value / max_value  # Scale the bar width as a percentage
  bar_style <- list(
    display = "inline-block",
    width = paste0(width_percent, "%"),  # Width of the bar changes based on value
    height = "15px",  # Bar height
    backgroundColor = color,
    whiteSpace = "nowrap",  # Prevents wrapping the text
    textAlign = "center",   # Aligns the number to the center inside the bar
    verticalAlign = "middle"  # Ensures vertical alignment is centered
  )
  # Ensure that the value is rendered correctly even when paginated
...
htmltools::tagList(
  ...
  # Create the table using reactable
  reactable(
    lane_miles_table,
    columns = list(
      ...
      state_tot_lane_miles = colDef(
        ...
        cell = function(value, index) {
          if (lane_miles_table$state[index] == "U.S. Total") {
            format(round(value), big.mark = ",", scientific = FALSE)  # Display the total as a rounded number without a bar
          } else {
            create_bar_chart(value, max_lane_miles, "#FFCC33")  # Yellow bar for lane miles
          }
        },
        minWidth = 300,  # Ensure enough width for the bar chart
    ...
    bordered = TRUE,
    highlight = TRUE,
    compact = TRUE,  # This reduces the overall height of the rows
    paginationType = "simple"  # Using simple pagination to avoid rendering issues
  )
)
...
# Prepare the data: select state and capital disbursement rank, add abbreviations
score_table <- scores_rankings %>%
  select(state, capital_disbursement_perlm_score_rank) %>%
  mutate(
    state_lower = tolower(state),  # Convert state names to lowercase for matching
    state_abb = state.abb[match(state, state.name)]  # Add state abbreviations
  )
...
# Define the custom color scale for rank ranges
rank_colors <- c(
...
ggplot() +
  ...
  labs(title = "Capital and Bridge Disbursements Per Lane-Mile Rank", fill = "Rank")
