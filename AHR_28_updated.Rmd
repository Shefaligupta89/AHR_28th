...
# Generate the reactable table with a title and bar chart, displaying all data
htmltools::tagList(
  # Add the title
...
# Prepare the data: select the necessary columns
...
# Function to create bar chart with different colors for each rank
...
# Add title and table
htmltools::tagList(
  # Aligning the title using CSS similar to your example
  htmltools::tags$div(
  ...
  # Create the table with bar charts for each rank
  reactable(
    data = table_data,
    columns = list(
...
# Merge map data with score_table using the lowercase state names
map_data <- left_join(states_map, score_table, by = c("region" = "state_lower"))

# Calculate the state centers for placing the text labels on the map
state_centers_df <- map_data %>%
  group_by(region) %>%
  summarise(long = mean(long), lat = mean(lat)) %>%
  left_join(score_table, by = c("region" = "state_lower"))

# Define colors for rank categories
rank_colors <- c(
...
# Function to categorize ranks into color-coded groups
...
# Plot the map showing Overall Highway Performance Rank
ggplot() +
...
# Bind U.S. Total to the lane_miles_table
...
# Prepare the data: Rank based on SHA_ratio and filter out "United States"
table_data <- AHR_data %>%
  filter(state != "United States") %>%  # Remove "United States"
  select(state, SHA_ratio, state_tot_lane_miles, SHA_miles) %>%
  filter(!is.na(SHA_ratio)) %>%
  arrange(desc(SHA_ratio)) %>%
  mutate(Size = row_number())  # Rank based on SHA_ratio

# Move the 'Size' column to be the first column
table_data <- table_data %>%
  select(Size, state, everything())  # Move 'Size' to the first column

# Calculate the max values for scaling bars
...
# Filter the data for capital disbursement per lane-mile
...
# Create a bar chart function (with inline-block styling for horizontal text)
create_bar_chart <- function(value, max_value, color = "#FFCC33") {
  width_percent <- 100 * value / max_value  
  bar_style <- list(
    display = "inline-block",
  ...
  # Create the bar and display the value next to it
  div(
    style = "display: flex; align-items: center;", 
    div(style = bar_style),  # Bar itself
...
# Get the max values for scaling the bars
...
# Load and process the data from the 'Scores & Rankings' sheet (assuming 'scores_rankings' is already loaded)
score_table <- Scores_and_Rankings %>%
...
# Join the map data with the score table
map_data <- left_join(states_map, score_table, by = c("region" = "state_lower"))

# Calculate state centers for adding text labels on the map
state_centers_df <- map_data %>%
  group_by(region) %>%
  summarise(long = mean(long), lat = mean(lat)) %>%
  left_join(score_table, by = c("region" = "state_lower"))

# Define the rank color categories
rank_colors <- c(
  "1 to 10" = "#66bb6a",  # Green
  "11 to 20" = "#b2df8a",  # Light Green
  "21 to 30" = "#ffeb3b",  # Yellow
...
# Function to categorize ranks for color mapping
categorize_rank <- function(rank) {
  if (is.na(rank)) {
    return("Not ranked")
  } else if (rank <= 10) {
...
# Apply the rank categorization function
map_data$rank_category <- sapply(map_data$capital_disbursement_perlm_score_rank, categorize_rank)

# Plot the Capital Disbursements Per Lane-Mile Rank map
ggplot() +
  geom_polygon(data = map_data, aes(x = long, y = lat, group = group, fill = rank_category), color = "white") +
  scale_fill_manual(values = rank_colors, breaks = names(rank_colors), na.value = "gray") +
  theme_minimal() +
  
  # Adding the state abbreviations and ranks as text on the map
...
# Filter the data for maintenance disbursement per lane-mile
...
# Filter the data for administrative disbursement per lane-mile
...
# Filter the data for other disbursement per lane-mile
